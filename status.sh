...
function save_downtime() {
	...
	if [[ "$BE_LOUD" = "yes" ]] || [[ "$BE_QUIET" = "no" ]]; then
		...
		if [[ $MY_COMMAND == "nc" ]] || [[ $MY_COMMAND == "nc6" ]]; then
...
<div class="pb-2 mt-5 mb-2 border-bottom">
	...
	<span class="text-muted">$MY_DATE_TIME</span>
</div>
...
<footer>
	<span class="text-muted">$MY_DATE_TIME</span>
	<span class="float-right d-none d-sm-block">$MY_STATUS_FOOTER</span>
</footer>
...
function item_ok() {
	...
	if [[ -n "${MY_DISPLAY_TEXT}" ]]; then
		...
		elif [[ "$MY_OK_COMMAND" = "nc" ]] || [[ "$MY_OK_COMMAND" = "nc6" ]]; then
...
function item_down() {
	...
	if [[ -n "${MY_DISPLAY_TEXT}" ]]; then
		...
		elif [[ "$MY_DOWN_COMMAND" = "nc" ]] || [[ "$MY_DOWN_COMMAND" = "nc6" ]]; then
...
function item_history() {
	...
	if [[ -n "${MY_DISPLAY_TEXT}" ]]; then
		...
		elif [[ "$MY_HISTORY_COMMAND" = "nc" ]] || [[ "$MY_HISTORY_COMMAND" = "nc6" ]]; then
...
while IFS=';' read -r MY_COMMAND MY_HOSTNAME_STRING MY_PORT || [[ -n "$MY_COMMAND" ]]; do
	...
	elif [[ "$MY_COMMAND" = "nc" ]] || [[ "$MY_COMMAND" = "nc6" ]]; then
		(( MY_HOSTNAME_COUNT++ ))
		if [[ "$MY_COMMAND" = "nc6" ]]; then
			MY_NC_IPV6="-6"
		fi
		if nc -z -w "$MY_TIMEOUT" "$MY_NC_IPV6" "$MY_HOSTNAME" "$MY_PORT" &> /dev/null; then
